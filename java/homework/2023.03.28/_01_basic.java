...
//		2. public static <T> String add(T t1, T t2) 제네릭 메소드를 
//		정의하고 매개변수로 어떤 값이 들어오던 문자열 결합연산이 돼서 스트링값으로 
//		리턴하도록 만드세요.
		System.out.println(_01_basic.add(1, 1.3));
		...
		for(T t : tArr) {
			if(t instanceof Integer ||
				...
				if((int)t % 3 == 0) {
					...
					t instanceof Float) {
				if(((Double)t).intValue() % 3 == 0) {
					System.out.println(t);
				}
			} else if(t instanceof String) {
				if(Integer.parseInt((String)t) % 3 == 0) {
					System.out.println(t);
				}
			} else {
				if(Integer.parseInt(t.toString()) % 3 == 0) {
					System.out.println(t);
				}
			}
		}
